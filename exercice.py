...
def list_to_dict(some_list: list) -> dict:
    ...
    return {element : indice for indice, element in enumerate(some_list)}


def color_name_to_hex(colors: list) -> list:
    # TODO: Trouver la valeur hex de chaque couleur dans la liste et créer une liste de tupple où le premier élément est le nom de la couleur et le deuxième est la valeur hex
    return [(name, cnames[name]) for name in colors]


def create_list() -> list:
    # TODO: Créer une liste des 10 000 premiers entiers positif, sauf pour les entiers de 15 à 350
    return [number for number in range(0,10000) if number < 15 or number > 350]


def compute_mse(model_dict: dict) -> dict:
    # TODO: Calculer l'erreur quadratique moyen pour chaque modèle. Retourner un dictionnaire contenant les MSE.
    return {model : sum([(element[0] - element[1])**2 / len(values) for element in values]) for model, values in model_dict.items()}
